    ...
    <div class="w-full max-w-md z-10 pt-10">
        <div id="login-view" class="bg-white/90 backdrop-blur-lg rounded-[2.5rem] shadow-2xl p-8 text-center border border-white/20">
            ...
            <div id="login-form" class="space-y-3 mt-6">
                ...
                <button onclick="login()" class="w-full bg-red-600 hover:bg-red-700 text-white font-black py-4 rounded-2xl shadow-lg transition-all active:scale-95 text-xs tracking-widest">ENTRAR</button>
                
                <a href="https://wa.me/584245834938?text=Hola!%20Olvidé%20o%20quiero%20cambiar%20mi%20código%20del%20Intercambio%202025" target="_blank" class="block text-[10px] font-bold text-green-700 uppercase tracking-tighter hover:underline mt-4">
                   ¿Olvidaste tu código o quieres cambiarlo? <br> <span class="text-xs">💬 Contactar Soporte</span>
                </a>
            </div>

            <div class="text-left border-t mt-8 pt-6">
                <p class="text-[9px] font-black text-gray-400 mb-4 uppercase text-center tracking-widest italic">Deseos</p>
                <div id="preview-list" class="max-h-40 overflow-y-auto custom-scroll space-y-2 pr-1"></div>
            </div>
        </div>
        ...
        <div id="app-view" class="hidden space-y-6">
            <div class="bg-white/95 backdrop-blur-md rounded-[2.5rem] p-8 shadow-xl border border-white/20 text-center relative">
                <button onclick="logout()" class="absolute top-8 left-8 text-gray-400 text-[10px] font-black uppercase hover:text-red-600 transition-colors">✕ Salir</button>
                
                <a href="https://wa.me/584245834938?text=Hola!%20Soy%20un%20usuario%20activo%20y%20quiero%20cambiar%20mi%20clave" target="_blank" class="absolute top-8 right-8 text-green-600 text-[10px] font-black uppercase hover:scale-105 transition-transform">🔑 Cambiar Clave</a>
        ...
        async function loadWishes() {
            ...
            if (container && !document.getElementById('app-view').classList.contains('hidden')) {
                ...
                container.innerHTML = Object.entries(grouped).map(([name, items]) => \`
                    <div class="bg-white/90 p-6 rounded-[2.5rem] shadow-xl border-l-[10px]" style="border-color:\${getColor(name)}">
                        <b class="text-[11px] font-black uppercase tracking-widest block mb-4" style="color:\${getColor(name)}">👤 \${name}</b>
                        <div class="space-y-2">
                            \${items.map(i => \`
                                <div class="flex justify-between items-center bg-white p-4 rounded-2xl text-xs shadow-sm border border-gray-50">
                                    <span class="text-gray-700 font-bold">\${i.wish}</span>
                                    \${name === currentUser ? \`<button onclick="deleteWish(\${i.id})" class="text-red-300 font-black px-2 hover:text-red-600">✕</button>\` : ''}
                                </div>
                            \`).join('')}
                        </div>
                    </div>
